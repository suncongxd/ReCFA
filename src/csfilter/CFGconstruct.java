...
public class CFGconstruct {
  ...
  // 存储化简后的控制流图
  ConcurrentHashMap<String, HashMap<String, Set<String>>> Reduction =
  ...
  public void ReadAsm(String AsmFile, CFG cfg, String outPath) {
    ...
    String longjmp = null;
    // 收集setjmpSet集合
    Set<String> setjmpSet = new HashSet<String>();
    // 收集间接调用集合
    Set<String> IndirectCallJump = new HashSet<String>();
        ...
        if (m1.find()) {
          ...
          if (!m2.find()) {
            if (cfg.BlockendJump.keySet().contains(m1.group(1))) {
              ...
              if (longjmp != null &&
                  cfg.BlockendJump.get(m1.group(1)).Jump.contains(longjmp))
                cfg.BlockendJump.get(m1.group(1)).Jump = null;
              else {
                if (setjmp != null &&
                    cfg.BlockendJump.get(m1.group(1)).Jump.contains(setjmp)) {
    ...
    Iterator<String> it;
    for (it = Call.iterator(); it.hasNext();) {
    ...
    for (it = P.iterator(); it.hasNext();) {
      ...
      /*
       * if(b.size()>=2) { Set<String> Insert=new HashSet<>();
       * Map<String,String> before=new HashMap<>(); //key是call跳转地址，value
       * for(Iterator it1=b.iterator();it1.hasNext();) { String
       * callAdress=(String) it1.next();
       * //调用地址
       * if(before.keySet().contains(callRet.CallTarget.get(callAdress))) {
       * Insert.add(callAdress); //System.out.println(callAdress+"
       * "+callRet.CallTarget.get(callAdress)+" "
       ...
       * else before.put(callRet.CallTarget.get(callAdress),callAdress); } }
       */
    ...
    try {
      ...
      pw2 = new PrintWriter(new FileWriter(outPath + ".map"));
      for (it = P.iterator(); it.hasNext();) {
        String a = (String)it.next();
        Set<String> b = Reduction.get(a).keySet();
      ...
      for (it = out.iterator(); it.hasNext();) {
  ...
  public void construct(CFG cfg) {
     ...
     * it=P.iterator();it.hasNext();) { String p = (String) it.next();
     * process(p, cfg); }
     */
    // 设计线程池，最多线程数为5
  ...
  public void process(String pHead, CFG cfg) {
    ...
    if (!Reduction.keySet().contains(pHead)) {
      ...
      for (Iterator<String> it = TailSet.iterator(); it.hasNext();) {
        String s = (String)it.next();
  ...
  // 深度优先遍历搜索
  public void dps(String pHead, CFG cfg, HashSet<String> flag,
                  HashSet<String> TailSet) {
    // 设计一个队列用来存储已经搜索过的基本块
