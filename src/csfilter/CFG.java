...
public class CFG {
		...
		try {
			...
			while (in != null) {
				...
				if (m.find()) {
					...
					insert.Jump = new TreeSet<String>();
					// key值为基本块的首部地址
		...
		try {
			...
			while (in != null) {
				...
				if (m.find()) {
					...
					if (next.size() != 2)
						throw new SetException("size mismatch.");
					next.remove("ffffffffffffffff");
					String NextBlock = "";
					for (Iterator<String> it = next.iterator(); it.hasNext();) {
					...
					for (Iterator<String> it = FunctionEnd.iterator(); it.hasNext();) {
			...
			for (Iterator<String> it = getFunctionEndJump().keySet().iterator(); it.hasNext();) {
	...
	public void dps(String s, String Star, Set<String> flag) {
		...
		// 对跳转目标集合元素进行分析
		Iterator<String> it = a.Jump.iterator();
		while (it.hasNext()) {
