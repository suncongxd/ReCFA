...
int main(int argc, char *argv[])
{
	...
	{
		printf("Wrong params, true Usage:\n1) If you want to deal with recursions and loops, type\n./allPress [fileName]\n2) If you just want to deal with loops, type\n./allPress -r [fileName]");
		return 0;
	}
    ...
    while ((rbyte = fread(&buffer, 4, 1, in)) == 1)
    {
        ...
        else if (buffer == 0xeeeeeeee)  
        {
            ...
            // circuStack.top()!= addrStack.stacktop进行判断的原因是循环的最后一次判断条件不满足，
            ...
            if (circuStack.top() != addrStack.stacktop)
            {
                ...
                while (cmpedPos != 0xffff)
                {
                    ...
                    cmpedLength = circuStack.stack[circuStack.stacktop - index + 1] - cmpedPos;
                    if (needCmpLength == cmpedLength && memcmp(addrStack.stack + needCmpPos, addrStack.stack + cmpedPos, needCmpLength * 4) == 0)
                    {
        ...
        }else if (dealWithRecursion && buffer == 0xaaaaffff)  
        {
            ...
            if (recStack.top() == 0xffff || recStack.stack[recStack.stacktop - 2] == 0xffff)
            {
                recStack.push(addrStack.stacktop);
                // printf("0xffff第一或者第二\n");
            } else {
                ...
                while (cmpedPos != 0xffff)
                {
                    cmpedLength = recStack.stack[recStack.stacktop - index + 1] - cmpedPos;
                    if (needCmpLength == cmpedLength && memcmp(addrStack.stack + needCmpPos, addrStack.stack + cmpedPos, needCmpLength * 4) == 0)
                    {
            ...
            if (addrStack.stacktop == addrStack.length)
            {
                ...
                else
                {
                    ...
                    if (circuStack.empty() && !recStack.empty()){
                        ...
                        circuStack.stack[1] <= recStack.stack[1] ? circuStack.stack[1] : recStack.stack[1];
                    } 
                    // 进行判断的原因是如果从地址栈的第一个开始就是处于循环中的，则只能增加栈的大小，栈的push操作可以完成
                    if (storeLen != 0)
                    {
                        printf("存在循环或者递归：需要保存的长度%d，成功输出到文件的长度%d\n",storeLen,fwrite(addrStack.stack, 4, storeLen, out));
                        ...
                        for (int i = 0; i < circuStack.stacktop; i++)
                        {
                            if (circuStack.stack[i] != 0xffff)
                            {
                                ...
                                if(circuStack.stack[i] < 0){
                                     printf("circuStack.stack[i]=%d,stooreLen=%d\n", circuStack.stack[i],storeLen);
                                }
                            ...
                            if (recStack.stack[i] != 0xffff)
                            {
                                recStack.stack[i] = recStack.stack[i] - storeLen;
                                 if(recStack.stack[i] < 0){
                                     printf("recStack.stack[i]=%d,stooreLen=%d\n", recStack.stack[i],storeLen);
                                }
                            }
                        }
                    }
